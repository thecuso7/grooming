	...
	<div>
		<div id="step-1" class="tw-bg-custom-main tw-rounded-xl tw-shadow-[0px_0px_15px_3px_rgba(0,_0,_0,_0.2)] tw-p-6 md:tw-p-8">
			...
			<div v-if="petList.length" class="tw-grid md:tw-grid-cols-2 tw-gap-6 tw-mb-6 tw-p-4">
				...
				<div v-for="pet in petList" @click="choose(pet.id)" :class="{'!tw-border-solid !tw-border-[#7D9E89]': pet.id === data.pet.id}" class="tw-border-2 tw-border-dashed tw-border-gray-300 tw-rounded-lg tw-p-4 tw-text-center tw-cursor-pointer hover:tw-border-solid hover:tw-border-[#7D9E89] tw-transition-colors">
					<v-img
						v-if="pet.image"
						:src="pet.image"
						max-width="300"
			...
			<button @click="add" v-if="!data.isNew" class="tw-w-full tw-py-3 tw-border-2 tw-border-dashed tw-border-gray-300 tw-rounded-lg tw-text-gray-600 hover:tw-border-solid hover:tw-border-[#7D9E89] tw-transition-colors tw-mb-6">
				<i class="fas fa-plus tw-mr-2"></i>Добавить нового питомца
			</button>

			<!-- Форма добавления питомца -->
			<div v-if="data.isNew" class="tw-bg-custom-purple tw-rounded-lg tw-p-6 tw-shadow-[0px_0px_15px_3px_rgba(0,_0,_0,_0.2)]">
				<h3 class="tw-font-medium tw-text-gray-800 tw-mb-4">Данные нового питомца</h3>
				
				<div class="tw-grid md:tw-grid-cols-2 tw-gap-4">
					<v-text-field
						v-model="data.pet.name"
						@change="v$.name.$touch"
			...
			<div class="tw-flex tw-justify-end tw-mt-6">
				<button @click="nextStep" class="tw-px-6 tw-py-2 tw-bg-custom-btn tw-text-white tw-rounded-lg tw-transition-colors">
...
<script setup>
	...
	const { v$ } = validate(data.pet);

	const choose = (id) => {
		data.isNew = false;
		data.pet.id = id;
		data.pet = petList.value.filter(pet => pet.id === id)[0];
	}
	...
	const nextStep = () => {
		if (v$.value.$invalid && !data.pet.id) {
            v$.value.$touch();
            return;
        }
	...
	watch(() => data, (newValue, oldValue) => {
		appointStore.updateData('choose', newValue);
	}, {deep: true});

	onMounted(async () => {
		if(appointStore.isSaved) {
			Object.assign(data, appointStore.stepsData.choose);
		}
