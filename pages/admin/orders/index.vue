<template>
    <div>
        <h1 class="tw-text-3xl tw-text-black tw-pb-6">Записи</h1>
        <div class="tw-w-full tw-mt-12 tw-flex tw-flex-col">
            <div class="tw-dropdown-wrap tw-self-end tw-relative">
                <!-- Добавить нового пользователя с кнопкой -->
                <button @click="toggleDropdown()" id="dropdownButton" class="tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-text-center tw-text-white tw-bg-gray-800 tw-rounded-lg hover:tw-bg-gray-600 focus:tw-ring-4 focus:tw-outline-none" type="button">
                ...
                <div v-click-outside:#dropdownButton="hadleClickOutside" id="dropdownSearch" class="tw-mt-2 tw-absolute tw-z-10 tw-rounded-lg tw-shadow-sm tw-w-60 tw-bg-gray-700 tw-right-0" :class="{'tw-hidden': !isOpenDropdown}">
                    <div class="tw-p-3">
                    ...
                    <ul class="tw-h-48 tw-px-3 tw-pb-3 tw-overflow-y-auto tw-text-sm tw-text-gray-700" aria-labelledby="dropdownSearchButton">
                        ...
                        <li>
                            <div class="tw-flex tw-items-center tw-p-2 tw-rounded-sm hover:tw-bg-gray-100 tw-group">
                                <input id="checkbox-item-11" type="checkbox" v-model="showCols.name" class="tw-w-4 tw-h-4 tw-text-blue-600 tw-bg-gray-100 tw-border-gray-300 tw-rounded-sm tw-focus:ring-blue-500">
                                <label for="checkbox-item-11" class="tw-w-full tw-ms-2 tw-text-sm tw-font-medium tw-text-white tw-rounded-sm group-hover:tw-text-gray-600">Имя</label>
                            </div>
                        </li>
                        <li>
                            <div class="tw-flex tw-items-center tw-p-2 tw-rounded-sm hover:tw-bg-gray-100 tw-group">
                                <input id="checkbox-item-11" type="checkbox" v-model="showCols.lastName" class="tw-w-4 h-4 tw-text-blue-600 tw-bg-gray-100 tw-border-gray-300 tw-rounded-sm focus:tw-ring-blue-500">
                                <label for="checkbox-item-11" class="tw-w-full ms-2 tw-text-sm tw-font-medium tw-text-white tw-rounded-sm group-hover:tw-text-gray-600">Фамилия</label>
                            </div>
                        </li>
                        <li>
                            <div class="tw-flex tw-items-center tw-p-2 tw-rounded-sm hover:tw-bg-gray-100 tw-group">
                                <input id="checkbox-item-11" type="checkbox" v-model="showCols.email" class="tw-w-4 tw-h-4 tw-text-blue-600 tw-bg-gray-100 tw-border-gray-300 tw-rounded-sm focus:tw-ring-blue-500">
                                <label for="checkbox-item-11" class="tw-w-full tw-ms-2 tw-text-sm tw-font-medium tw-text-white tw-rounded-sm group-hover:tw-text-gray-600">Email</label>
                            </div>
                        </li>
                        <li>
                            <div class="tw-flex tw-items-center tw-p-2 tw-rounded-sm hover:tw-bg-gray-100 tw-group">
                                <input id="checkbox-item-11" type="checkbox" v-model="showCols.role" class="tw-w-4 tw-h-4 tw-text-blue-600 tw-bg-gray-100 tw-border-gray-300 tw-rounded-sm focus:tw-ring-blue-500">
                                <label for="checkbox-item-11" class="tw-w-full tw-ms-2 tw-text-sm tw-font-medium tw-text-white tw-rounded-sm group-hover:tw-text-gray-600">Роль</label>
                            </div>
                        </li>
                    </ul>
                </div>
            ...
            <v-table theme="white" class="rounded-lg">
                <thead>
                    <tr class="tw-bg-gray-800 tw-text-white tw-uppercase">
                ...
                <tbody class="tw-text-gray-700">
                    <tr v-for="(user,key) in users" class="tw-bg-gray-100" :class="{'tw-bg-gray-300' : (key + 1) % 2 == 0}">
                        ...
                        <td v-if="showCols.name" class="tw-w-1/3 tw-text-left tw-py-3 tw-px-4">{{ user.name }}</td>
                        <td v-if="showCols.lastName" class="tw-w-1/3 tw-text-left tw-py-3 tw-px-4">{{ user.lastName }}</td>
                        <td v-if="showCols.email" class="tw-text-left tw-py-3 tw-px-4">
                            <NuxtLink
                        ...
                        <td v-if="showCols.email" class="tw-text-left tw-py-3 tw-px-4">{{ user.role }}</td>
                    </tr>
                </tbody>
            </v-table>
            ...
            <!-- <div class="bg-white overflow-auto rounded-lg mt-6">
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 text-white">
                        <tr>
                            <th v-if="showCols.id" class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">ID</th>
                            <th v-if="showCols.name" class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Имя</th>
                            <th v-if="showCols.lastName" class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Фамилия</th>
                            <th v-if="showCols.email" class="text-left py-3 px-4 uppercase font-semibold text-sm">Email</th>
                            <th v-if="showCols.role" class="text-left py-3 px-4 uppercase font-semibold text-sm">Роль</th>
                        </tr>
                    </thead>
...
<script setup>
    ...
    const loadUsers = async (params) => {
        const response = await $api(`/api/admin/users?page=${params.page}`);
